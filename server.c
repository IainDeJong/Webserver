...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* remainder = &line[4];
                        
            char* HTTP_version = strchr(remainder, ' '); //or strstr?
            ...
            //if (strchr(HTTP_version, ' ') != NULL)
              //  {
                //error(400);
                //continue;
                //}
            
            char request_target[HTTP_version - remainder + 1]; 
            strncpy(request_target, remainder, HTTP_version - remainder);
            request_target[HTTP_version - remainder] = '\0';
            ...
            if (strchr(remainder, '?') != NULL)
                {
                query_size = (HTTP_version - strchr(remainder, '?') + 1);
                }
            else
                {
                query_size = 1;
            ...
            // = strchr(request_target, '?') != NULL ? strchr(request_target, '?') : '\0';
           
          // char* jocko = strchr(request_target, '?');
            
            
            
           //char query[5000] = strchr(request_target, '?');
            ...
            char request_target_copy[sizeof(request_target)];
            strcpy(request_target_copy, request_target);
            if (strchr(request_target, '?') != NULL)
                {
                absolutepathsize = (strchr(request_target_copy, '?') - request_target_copy) + 1;
                }
            
            else
                {
                absolutepathsize = sizeof(request_target_copy);
                }
            
            char absolute_path[absolutepathsize];
            
            strncpy(absolute_path, request_target_copy, absolutepathsize);
            absolute_path[absolutepathsize-1] = '\0';
            
            
            
            char path[strlen(absolute_path) + strlen(root) + 1]; 
            strcpy(path, root);
